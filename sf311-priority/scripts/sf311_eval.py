...
def parse_args():
    ...
    ap.add_argument("--fail-examples", type=int, default=5, help="Examples per failing test")
            ...
            if ok:
                ...
                if len(examples[name]) < k_examples:
                    examples[name].append({
                        ...
                        "derived_is_private_property": r.get("derived_is_private_property"),
                    })
    return {
        "totals": totals,
        "passes": passes,
        "pass_rates": {k: (passes[k] / totals[k] if totals[k] else None) for k in totals},
